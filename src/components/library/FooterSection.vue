<template>
    <footer class="custom-footer-section" id="footer">
      <!-- Footer Top -->
      <div class="footer-top bg-name-primary section-padding">
        <!-- Shape Start -->
        <div class="footer-shape-left layer-shape"></div>
        <div class="footer-shape-right layer-shape"></div>
        <div class="container">
          <div class="row m-b-n40">
            <div class="col-12 col-md-6 col-lg-3 col-xl-4 m-b-40 aos-init aos-animate" data-aos="fade-up" data-aos-duration="1000">
                <div class="single-footer-widget">
                    <div
                        class="footer-logo m-b-20 editable"
                        @click="selectField('logo')"
                        @mouseover="hoveredField = 'logo'"
                        @mouseleave="hoveredField = null"
                        :class="{ selected: selectedField === 'logo' }"
                    >
                        <span v-if="hoveredField === 'logo'" class="edit-label">Logo</span>
                        <input
                        v-if="selectedField === 'logo'"
                        type="file"
                        @change="handleLogoUpload"
                        @blur="selectedField = null"
                        />
                        <img
                        v-else
                        :src="editableContent.logo"
                        alt="Footer Logo"
                        class="footer-logo-img"
                        />
                    </div>
    
                    <!-- Description -->
                    <div
                        class="editable"
                        :class="{ selected: selectedField === 'description' }"
                        @click="selectField('description')"
                        @mouseover="hoveredField = 'description'"
                        @mouseleave="hoveredField = null"
                    >
                        <span v-if="hoveredField === 'description'" class="edit-label">Text</span>
                        <textarea
                        v-if="selectedField === 'description'"
                        v-model="editableContent.description"
                        @blur="blurAndUpdate"
                        ></textarea>
                        <p v-else class="footer-desc desc-content m-b-35">{{ editableContent.description }}</p>
                    </div>
    
                    <!-- Button -->
                    <div
                        class="appoinment-button editable"
                        :class="{ selected: selectedField === 'buttonText' }"
                        @click="selectField('buttonText')"
                        @mouseover="hoveredField = 'buttonText'"
                        @mouseleave="hoveredField = null"
                        v-if="editableContent.buttonText"
                    >
                        <span v-if="hoveredField === 'buttonText'" class="edit-label">Text</span>
                        <input
                        v-if="selectedField === 'buttonText'"
                        v-model="editableContent.buttonText"
                        @blur="blurAndUpdate"
                        />
                        <a v-else class="btn btn-outline-success btn-hover-success" href="#">
                        {{ editableContent.buttonText }} 
                        </a>
                    </div>
                </div>
            </div>
  
            <!-- Column 2: Contact Heading -->
            <div class="col-sm-6 col-md-6 col-lg-2 col-xl-2 m-b-40 aos-init aos-animate" data-aos="fade-up" data-aos-duration="1500">
                <div class="single-footer-widget footer-menu ">
                    <div
                        class="editable"
                        :class="{ selected: selectedField === 'contactHeading' }"
                        @click="selectField('contactHeading')"
                        @mouseover="hoveredField = 'contactHeading'"
                        @mouseleave="hoveredField = null"
                    >
                        <span v-if="hoveredField === 'contactHeading'" class="edit-label">Heading</span>
                        <input
                        v-if="selectedField === 'contactHeading'"
                        v-model="editableContent.contactHeading"
                        @blur="blurAndUpdate"
                        />
                        <h4 v-else class="widget-title footer-title">{{ editableContent.contactHeading }}</h4>
                    </div>
                    <ul class="footer-list">
                        <li><a href="#">{{ editableContent.phone }}</a></li>
                        <li><span>{{ editableContent.address }}</span></li>
                    </ul>
                </div>
            </div>
  
            <!-- Column 3: Menu Heading -->
            <div class="col-sm-6 col-md-6 col-lg-2 col-xl-2 m-b-40 aos-init aos-animate" data-aos="fade-up" data-aos-duration="2000">
                <div class="single-footer-widget footer-menu">
                    <div
                        class="editable"
                        :class="{ selected: selectedField === 'menuHeading' }"
                        @click="selectField('menuHeading')"
                        @mouseover="hoveredField = 'menuHeading'"
                        @mouseleave="hoveredField = null"
                    >
                        <span v-if="hoveredField === 'menuHeading'" class="edit-label">Heading</span>
                        <input
                        v-if="selectedField === 'menuHeading'"
                        v-model="editableContent.menuHeading"
                        @blur="blurAndUpdate"
                        />
                        <h4 v-else class="widget-title footer-title">{{ editableContent.menuHeading }}</h4>
                    </div>
                    <ul class="footer-list">
                        <li v-for="(item, i) in editableContent.menu" :key="i">
                        <a href="#">{{ item }}</a>
                        </li>
                    </ul>
                </div>
            </div>
  
            <!-- Column 4: Social Heading -->
            <div class="col-sm-6 col-md-6 col-lg-3 col-xl-2 m-b-40 aos-init aos-animate" data-aos="fade-up" data-aos-duration="2500">
                <div class="single-footer-widget single-footer-space-left">
                    <!-- Editable Social Heading -->
                    <div
                    class="editable"
                    :class="{ selected: selectedField === 'socialHeading' }"
                    @click="selectField('socialHeading')"
                    @mouseover="hoveredField = 'socialHeading'"
                    @mouseleave="hoveredField = null"
                    >
                    <span v-if="hoveredField === 'socialHeading'" class="edit-label">Heading</span>
                    <input
                        v-if="selectedField === 'socialHeading'"
                        v-model="editableContent.socialHeading"
                        @blur="blurAndUpdate"
                    />
                    <h4 v-else class="widget-title footer-title">{{ editableContent.socialHeading }}</h4>
                    </div>

                    <!-- Just Icons -->
                    <ul class="footer-social">
                    <li v-for="(icon, i) in editableContent.socialLinks" :key="i">
                        <i :class="icon" aria-hidden="true"></i>
                    </li>
                    </ul>
                </div>
            </div>
          </div>
        </div>

        <!-- Footer Bottom -->
        <div class="footer-bottom bg-name-primary p-t-20 p-b-20">
            <div class="container">
                <div class="row">
                <div class="col-md-6 m-b-10">
                    <div class="copyright-content">
                    <p class="mb-0">
                        © {{ new Date().getFullYear() }}
                        <strong class="text-color-success">Your Agency</strong>
                    </p>
                    </div>
                </div>
                <div class="col-md-6 m-b-10 site-made-by">
                    <div class="site-by">
                    <p>
                        site by <i class="icofont-heart-alt text-color-success"></i>
                        <span class="text-color-success">SpeedySites.</span>
                    </p>
                    </div>
                </div>
                </div>
            </div>
        </div>
        </div>
    </footer>
  </template>
  
  <script setup>
  import { ref, watch } from "vue";
  
  const props = defineProps({
    data: {
      type: Object,
      default: () => ({
        logo: "/images/logo.png",
        description: "Default footer description goes here.",
        buttonText: "Book Now",
        phone: "+91 99999 99999",
        address: "123 Street, City, State, Country",
        contactHeading: "Contact",
        menuHeading: "Quick Links",
        socialHeading: "Follow Us",
        menu: ["Home", "About", "Services", "Contact"],
        socialLinks: ["fa fa-facebook", "fa fa-instagram"],
        copyright: "© 2025 Your Agency. Site by SpeedySites.",
      }),
    },
  });
  
  const emit = defineEmits(["update"]);
  const editableContent = ref({ ...props.data });
  
  watch(
    () => props.data,
    (newVal) => {
      editableContent.value = { ...newVal };
    },
    { immediate: true, deep: true }
  );
  
  const selectedField = ref(null);
  const hoveredField = ref(null);
  
  function selectField(field) {
    selectedField.value = field;
  }
  
  function blurAndUpdate() {
    emit("update", editableContent.value);
    selectedField.value = null;
  }
  
  function handleLogoUpload(event) {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => {
        editableContent.value.logo = e.target.result;
        emit("update", editableContent.value);
        selectedField.value = null;
      };
      reader.readAsDataURL(file);
    }
  }
  </script>

<style scoped>
 /* Editable styling */
 .editable {
    cursor: pointer;
    padding: 4px;
    position: relative;
    border: 2px dashed transparent;
    transition: border-color 0.2s ease;
  }
  
  .editable:hover {
    border-color: #00adb5;
  }
  
  .editable.selected {
    border-color: #00adb5;
  }
  
  .edit-label {
    position: absolute;
    top: -10px;
    left: 5px;
    background-color: #00adb5;
    color: white;
    font-size: 10px;
    padding: 2px 6px;
    border-radius: 3px;
    z-index: 2;
    pointer-events: none;
  }
  
  input,
  textarea {
    width: 100%;
    font-size: inherit;
    font-family: inherit;
    padding: 8px;
    border: 1px solid #ccc;
  }
ul.widget-list {
    margin: 0;
    padding: 0;
}
.single-footer-widget .table-list li {
    color: #00ADB5;
    font-size: 14px;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-align: center;
    -webkit-align-items: center;
    -ms-flex-align: center;
    align-items: center;
}
.footer-top {
    position: relative;
    overflow: hidden;
}
.bg-name-primary {
    background-color:#222831;
}
.section-padding {
    padding-top: 35px;
}

.footer-top .layer-shape.footer-shape-left {
    top: 0;
}
.footer-top .layer-shape {
    -webkit-animation: movebounce 2s infinite alternate;
    animation: movebounce 2s infinite alternate;
    position: absolute;
}
.footer-top .layer-shape.footer-shape-left::before {
    /* top: 150px; */
    /* left: -100px; */
}
.single-footer-widget .table-list li span:hover {
    color: #00adb5;
}
.footer-top .layer-shape.footer-shape-left::before {
    content: "";
    position: absolute;
    border: 2px solid #00adb5;
    width: 190px;
    height: 115px;
    border-radius: 50%;
    -webkit-transform: rotate(-20deg);
    -ms-transform: rotate(-20deg);
    transform: rotate(-20deg);
    left: -80px;
    top: 90px;
}
.footer-top .layer-shape.footer-shape-left::after {
    content: "";
    position: absolute;
    border: 2px solid #00adb5;
    width: 186px;
    height: 120px;
    border-radius: 50%;
    -webkit-transform: rotate(-20deg);
    -ms-transform: rotate(-20deg);
    transform: rotate(-20deg);
    left: -80px;
    top: 145px;
}
.footer-top .layer-shape.footer-shape-right {
    bottom: 0;
    right: 0;
}

.footer-top .layer-shape {
    -webkit-animation: movebounce 2s infinite alternate;
    animation: movebounce 2s infinite alternate;
    position: absolute;
}
.footer-top .layer-shape.footer-shape-right::before {
    bottom: 150px;
    right: -100px;
}
.footer-top .layer-shape.footer-shape-right::before {
    content: "";
    position: absolute;
    border: 2px solid #00adb5;
    width: 190px;
    height: 90px;
    border-radius: 50%;
    -webkit-transform: rotate(-20deg);
    -ms-transform: rotate(-20deg);
    transform: rotate(-20deg);
    right: -80px;
    bottom: 100px;
}
.single-footer-widget .widget-list li a:hover {
    color:#00adb5;
}
.single-footer-widget .widget-list li a:hover i {
    font-size: 14px;
    padding-right: 5px;
    -webkit-transition: all 0.3s ease 0s;
    -o-transition: all 0.3s ease 0s;
    transition: all 0.3s ease 0s;
}
.footer-logo span.text-dark{
    font-size: 24px; 
    color: #F9FAFB !important;  
}
a.text-color-success:hover {
    color: #F9FAFB !important;  
}
.footer-top .layer-shape.footer-shape-right::after {
    content: "";
    position: absolute;
    border: 2px solid #00adb5;
    width: 190px;
    height: 90px;
    border-radius: 50%;
    -webkit-transform: rotate(-20deg);
    -ms-transform: rotate(-20deg);
    transform: rotate(-20deg);
    right: -80px;
    bottom: 160px;
}
.m-b-20 {
    margin-bottom: 20px;
}
.single-footer-widget .desc-content {
    color: #F9FAFB;
    width: 280px;
    font-size: 18px;
    line-height: 2;
}

.m-b-35 {
    margin-bottom: 35px;
}

a.btn.btn-outline-success.btn-hover-success {
    color:#393E46;
    background-color: #FFFFFF;
    border: 2px solid #FFFFFF;
    border-radius: 6px;
    padding: 12px 24px;
    font-size: 18px !important;
    font-weight: 500 !important;
}

a.btn.btn-outline-success.btn-hover-success:hover {
    color:#FFFFFF;
    background-color: #393E46;
    border: 2px solid #FFFFFF;
}

.btn-hover-success i {
    font-size: 14px;
    vertical-align: middle;
    line-height: 14px;
    margin-left: 20px;
}
.single-footer-widget .table-list li span {
    max-width: 120px;
    width: 100%;
    color: #F9FAFB;
}
.single-footer-widget .table-list li .table-list-middle {
    width: 80px;
}

.single-footer-widget .widget-title {
    font-size: 18px;
    color:#F9FAFB;
    margin-bottom: 25px;
    font-weight: 500;
    /* font-family: "Roboto", sans-serif; */
}
.single-footer-widget .widget-list li {
    margin-bottom: 20px;
    list-style: none;
}
span.text-color-success{
    color: #00adb5;
}
.single-footer-widget .widget-list li a {
    color: #F9FAFB;
    font-size: 14px;
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-align: center;
    -webkit-align-items: center;
    -ms-flex-align: center;
    align-items: center;
    text-decoration: none;
}
.single-footer-widget .widget-list li a i {
    font-size: 0px;
    padding-right: 0px;
    -webkit-transition: all 0.3s ease 0s;
    -o-transition: all 0.3s ease 0s;
    transition: all 0.3s ease 0s;
}

.single-footer-widget.single-footer-space-left {
    padding-left: 20px;
}

.single-footer-widget.single-footer-space-left {
    padding-left: 20px;
}

.footer-bottom {
    border-top: 1px solid #222831;
}

.p-b-20 {
    padding-bottom: 20px;
}
.p-t-20 {
    padding-top: 20px;
}

/* .bg-name-primary {
    background-color:var(--c4e-primaryColor-1);
} */

.copyright-content {
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-pack: start;
    -webkit-justify-content: flex-start;
    -ms-flex-pack: start;
    justify-content: flex-start;
}

.copyright-content p {
    color:#F9FAFB;
    font-size: 15px;
    margin-bottom: 0;
}
.text-color-success {
    color: #00adb5;
    text-decoration: none;
}
.footer-menu ul {
    display: -webkit-box;
    display: -webkit-flex;
    display: -ms-flexbox;
    display: flex;
    -webkit-box-pack: end;
    -webkit-justify-content: flex-end;
    -ms-flex-pack: end;
    justify-content: flex-end;
    list-style: none;
    gap: 10px;
    padding: 0;
    flex-direction: column;
}
.footer-menu ul li {
    position: relative;
    margin-right: 10px;
    padding-right: 20px;
}
.footer-menu ul li a {
    color:#F9FAFB;
    text-decoration: none;
}
span.name-tag {
    color: #222831;
}
.footer_logo .logo {
    max-width: 25% !important;
    height: auto !important;
}
.list-footer a:hover {
    color: #00adb5;
}
.list-footer a {
    color: #FFFFFF;
}
.list-footer {
    display: flex;
    flex-direction: row;
    gap: 15px;
    font-size: 20px;
}
.site-by p {
    color: #fff;
}
.site-made-by {
    display: flex;
    justify-content: end;
}
.footer-social {
  display: flex; 
  gap: 15px;
  list-style: none;
  padding: 0;
  margin: 0;
}

.footer-social li {
  display: inline-block;
}

.footer-social li i {
  color: #fff; 
  font-size: 18px;  
  transition: color 0.3s ease;
}
</style>